...
public class GlobalRoll extends Command {
    ...
    public void execute(String command, CommandSender sender, String[] args) {
        switch (args[0].toLowerCase()) {
            case "dice":
                if (args.length == 2) {
                    fallout.getRollManager().rollDice((Player) sender, args[1], RollManager.Distance.GLOBAL);
                } else {
                    fallout.getMessenger().sendMessage(sender, FOMessage.COMMAND_USAGE, "/fo globalroll dice <amount>d<sides>[+/-modifier]");
                }
                break;
            case "armor":
                if (args.length == 2) {
                    fallout.getRollManager().rollArmor((Player) sender, args[1], RollManager.Distance.GLOBAL);
                } else {
                    fallout.getMessenger().sendMessage(sender, FOMessage.COMMAND_USAGE, "/fo globalroll armor <damage type>[+/-modifier]");
                }
                break;
            default:
                fallout.getRollManager().rollDefault((Player) sender, args[0], RollManager.Distance.GLOBAL);
        }
    }
